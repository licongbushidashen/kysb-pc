<template>
  <div class="c-container">
    <flexbox
      v-if="!$slots.icon && showTitle" class="title"
    ><img :src="titleIcon" class="title-icon" >{{ title }}</flexbox
    >
    <slot name="icon" />
    <el-input
      v-if="showSearch"
    ...
    >
      <el-button
        slot="append" type="primary" @click.native="searchInput"
      >搜索</el-button
      >
    </el-input>
...
export default {
  ...
  computed: {
    ...
    showDupCheck() {
      return (
        ['leads', 'customer', 'contacts'].includes(this.crmType) && !this.isSeas
      )
    },
    ...
    moreTypes() {
      const moreTypes = []
      let importAuth =
        this.crm[this.crmType] && this.crm[this.crmType].excelimport
      ...
      let exportAuth =
        this.crm[this.crmType] && this.crm[this.crmType].excelexport
      if (this.isSeas && this.poolId) {
