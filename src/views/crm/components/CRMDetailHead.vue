    ...
    <flexbox class="t-section" align="stretch">
      ...
      <div class="t-section__bd">
        ...
        ><el-tooltip
          ...
          placement="top-start"
        >
        <div class="name">{{ name }}</div> </el-tooltip
        ><slot name="name" />
          <el-button-group
            v-if="pageList && pageList.length > 1"
            class="wk-header-page-btn"
          >
            <el-button
      ...
      <div class="t-section__ft">
        ...
        <el-button
          ...
          icon="wk wk-receive"
          type="primary"
          @click.native="handleTypeClick('get')"
        >领取</el-button
        >
        ...
        <el-button
          ...
          type="primary"
          icon="wk wk-alloc"
          @click.native="handleTypeClick('alloc')"
        >分配</el-button
        >
        ...
        <el-button
          ...
          type="primary"
          icon="wk wk-success"
          @click.native="handleTypeClick('deal_status')"
        >更改成交状态</el-button
        >
...
import { crmLeadsTransformAPI, crmLeadsDeleteAPI } from '@/api/crm/leads'
import {
  crmCustomerLockAPI,
...
import { crmContractDeleteAPI, crmContractCancelAPI } from '@/api/crm/contract'
...
import { crmProductStatusAPI, crmProductDeleteAPI } from '@/api/crm/product'
  ...
  methods: {
    ...
    handleTypeClick(type) {
      if (type == 'edit') {
        ...
        const routeData = this.$router.resolve(
          `/print/?module=${this.crmType}&id=${id}`
        )
    ...
    whetherTypeShowByPermision(type) {
      if (type == 'transfer') {
        ...
        // 合同作废 1 通过 10 正常
        if (
          this.crm[this.crmType].discard &&
          (this.detail.checkStatus === 1 || this.detail.checkStatus === 10)
        ) {
          return true
        }
        return false
