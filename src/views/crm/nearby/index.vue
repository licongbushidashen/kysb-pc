  ...
  <div class="map-box">
    ...
    <div v-loading="loading" class="map-content">
            ...
            <div class="type-popper">
              <div class="type-content">
                <div
                  ...
                  :class="{
                    selected:
                      selectType.value == item.value && !showCustomContent
                  }"
              ...
              <div v-if="showCustomContent" class="type-content-custom">
                <el-input v-wk-number="'positiveInt'" v-model="mapData.radius">
                  <template
                    slot="append"
                  >米</template
                  >
                ...
                <el-button
                  type="primary" @click="radiusChange(null)"
                >确定</el-button
                >
              </div>
            </div>
      ...
      <flexbox align="stretch" class="box-content">
        <div v-empty="mapList" xs-empty-icon="none" class="map-content--left">
          <div id="map-scroll">
            <ul ref="nearbyList" :style="{ height: mapHeight + 'px' }">
              <li
                ...
                :style="{
                  backgroundColor:
                    item.customerId === currentId ? '#f7f7f7' : ''
                }"
                class="map-info--box"
              ...
              >
                ...
                <div class="map-box--content">
                  <div class="map-info--name">
                    ...
                    >
                      <div
                        class="map-customer-name can-visit--underline"
                        @click.stop="checkCustomerDetail(item.customerId)"
                      >
                        {{ item.customerName }}
                      </div>
                    </el-tooltip>
                  ...
                  <div :title="item.detailAddress" class="map-info--content">
                    {{ item.detailAddress }}
                  </div>
                  <div class="map-info--content">
                    距离选择地址: {{ item.distance / 1000 }} km
                  </div>
                </div>
              </li>
            </ul>
...
export default {
  ...
  methods: {
    ...
    getMapInfo() {
      ...
      crmCrmCustomerNearbyCustomerAPI(params)
        .then(res => {
    ...
    addMarkerLabel() {
      ...
      ComplexCustomOverlay.prototype.initialize = function(map) {
        ...
        div.style.zIndex = this._zIndex = BMap.Overlay.getZIndex(
          this._point.lat
        )
      ...
      ComplexCustomOverlay.prototype.draw = function() {
        ...
        this._div.style.left =
          pixel.x - parseInt(this._arrow.style.left) - 10 + 'px'
        this._div.style.top = pixel.y - 30 + 'px'
      }
      const self = this
      this.mapList.forEach(item => {
        const itemPoint = new BMap.Point(item.lng, item.lat)
        const Marker = new ComplexCustomOverlay(
          itemPoint,
          item.customerName,
          item.customerId
        )
