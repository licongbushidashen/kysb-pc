<template>
  <div class="main">
    <xr-header
      icon-class="iconfont icon-huopin-"
      icon-color="#2362FB"
      label="货品管理" >
      <template v-slot:ft>
        <el-button
          v-if="allAuth['OrderSetting.GoodsInfos.Create']"
          class="main-table-header-button "
          type="primary"
          icon="el-icon-plus"
          @click="addJurisdiction">新建</el-button>
        <el-button
          v-if="allAuth['OrderSetting.GoodsInfos.Import']"
          class="main-table-header-button "
          type=""
        ...
        <el-button
          v-if="allAuth['OrderSetting.GoodsInfos.Delete']"
...
export default {
   ...
   */
    onItemCheckboxChange() {
      ...
      this.obj = {}
      this.ids = this.list.filter((d) => d.checked).map(e => {
        return e.id
      })
    ...
    handleRowClick(row, column, event) {
      if (!this.allAuth['OrderSetting.GoodsInfos.Edit']) {
