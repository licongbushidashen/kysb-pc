<template>
  ...
  >
    <el-button
      type="warning" icon="el-icon-plus" @click="createEvents"
    >新建日程</el-button
    >
    <div class="box-left">
      <div class="left-title">
        ...
        <wk-user-select
          ...
          ref="wkUserSelect"
          :value="
            checkedUser && checkedUser.length > 0 ? checkedUser[0].userId : ''
          "
          ...
          <flexbox slot="reference" class="user-box">
            <span class="username">{{
              checkedUser.length > 0
                ? checkedUser[0].realname + '的日程'
                : '我的日程'
            }}</span>
            <span
              :class="{
                'is-reverse': $refs.wkUserSelect && $refs.wkUserSelect.visible
              }"
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      canlendarQueryListAPI(this.activeTime)
        .then(res => {
          this.calendarEvents = []
    ...
    datesRender(info) {
      ...
      if (info.view.type === 'listDay') {
        ...
        if (this.currentTime === info.view.title) {
          if (
            this.selectDiv &&
            document.querySelector(`td[data-date="${this.selectDiv}"]`)
          ) {
            // 保证切换模式时，关联的日期被选中
            document
              .querySelector(`td[data-date="${this.selectDiv}"]`)
              .classList.add('select-day')
          }
        } else {
        ...
        if (
          ...
          new Date(info.view.activeStart).getTime()
        ) {
    ...
    getTask() {
      ...
      canlendarEventTaskAPI(params)
        .then(res => {
          const resData = res.data || []
          this.taskList = resData.map(item => {
            return {
              title: item.name,
              startTime: moment(item.startTime - 0 + 1000).format(
                'YYYY-MM-DD HH:mm:ss'
              ),
              id: item.taskId,
              eventId: item.taskId,
              ...
              endTime: moment(item.endTime - 0 + 1000).format(
                'YYYY-MM-DD HH:mm:ss'
              )
            }
          })
    ...
    handleData(list) {
      ...
      const leadsObj =
        this.sysTypeId.find(item => item.crmType === 'leads') || {}
      ...
      const customerObj =
        this.sysTypeId.find(item => item.crmType === 'customer') || {}
      ...
      const businessObj =
        this.sysTypeId.find(item => item.crmType === 'business') || {}
      ...
      const dealBusinessObj =
        this.sysTypeId.find(item => item.crmType === 'dealBusiness') || {}
      ...
      const endContractObj =
        this.sysTypeId.find(item => item.crmType === 'contract') || {}
      ...
      const receiveContractObj =
        this.sysTypeId.find(item => item.crmType === 'receiveContract') || {}
