<template>
  ...
  >
    <div class="dialog-body">
      ...
      <div v-if="stepsActive == 1" class="sections">
        <div>请选择需要导入的文件</div>
        <div class="content">
        ...
        <div class="download" @click="download">
          点击下载{{ type ? '《部门导入模板》' : '《员工导入模板》' }}
        </div>
        <div class="content content-tips">
          <div>操作步骤：</div>
          <div>1、下载{{ type ? '《部门导入模板》' : '《员工导入模板》' }}</div>
          <div>
            2、打开《员工导入模板》将对应字段信息输入或粘贴进本表。为保障粘贴信息被有效导入，请使用纯文本或数字
          </div>
      ...
      <div v-loading="loading" v-else-if="stepsActive == 3" class="sections">
        <div class="result-info">
          ...
          <!-- <p class="result-info__detail">
            ...
            >条，导入成功<span
              class="result-info__detail--suc"
            ><template v-if="resultData">{{
              resultData.totalSize - (resultData.errSize || 0)
            }}</template></span
          ...
          <el-button
            v-if="resultData && resultData.errorMsgList > 0"
