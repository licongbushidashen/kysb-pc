<template>
  <div class="setting-options">
    <draggable :list="optionsList" :options="dragConfig" @sort="handleChange">
      <div
    ...
    <div v-if="showOther" class="option-item other-item">
      <el-input value="其他" disabled>
        <flexbox v-if="optionsEditAuth" slot="suffix">
          <el-button
            type="text"
            class="el-input__icon wk wk-icon-bin"
    ...
    <el-button v-if="optionsEditAuth" class="add-btn" @click="handleAdd">
      <i class="el-icon-plus" /> 添加新选项
    </el-button>

    <flexbox v-if="optionsEditAuth" align="center" justify="center">
    ...
    handleChange() {
      ...
      if (this.field.remark === 'options_type') {
        ...
        Object.keys(optionsData).forEach(key => {
          const findRes = this.optionsList.find(
            o => o.value === key && key !== '其他'
          )
          if (!findRes) {
    ...
    checkDefaultValue() {
      if (!isEmpty(this.field.defaultValue)) {
        if (isArray(this.field.defaultValue)) {
          ...
          const findRes = this.optionsList.find(
            item => item.value === this.field.defaultValue
          )
          if (!findRes) {
